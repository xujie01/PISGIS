...
define([
    ...
    'dojo/promise/all',
    './utils',
        'jimu/magicgis/util/layers/GoogleLayer',
        'jimu/magicgis/util/layers/TianDiTuLayer',
  ...
  function(
    ...
    on,
    all,
    utils,GoogleLayer,TianDiTuLayer,A4MapLayer) {
    var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      selectionChange: function() {
        ...
        if (this.isOnScreen) {
          ...
          //加载地图底图
          for(var j = this.map.layerIds.length-1; j >= 0; j--) {//移除底图，BasemapGallery已经移除一个
              var layer = this.map.getLayer(this.map.layerIds[j]);
              if(layer.type == "googlepoi" ||layer.type == "googlemap" || layer.type == "googleimage" || layer.type == "googletrain"||
                  layer.type == "tianditumapi" || layer.type == "tiandituimagei" || layer.type == "tianditutraini"||layer.type == "tianditumap" || layer.type == "tiandituimage" || layer.type == "tianditutrain"||
                  layer.type == "a4mapi" || layer.type == "a4imagei" || layer.type == "a4traini"||layer.type=="a4map"||layer.type=="a4image"||layer.type=="a4train")
              {
                  this.map.removeLayer(this.map.getLayer(this.map.layerIds[j]));
              }
          }
          var basemap = this.basemapGallery.getSelected();
          var layer;
          if (basemap.layers[0].type == "googlemap" || basemap.layers[0].type == "googleimage" || basemap.layers[0].type == "googletrain") {
              layer = new GoogleLayer();
              layer.type = basemap.layers[0].type;
              layer.label = basemap.title;
              this.map.addLayer(layer, 0);

              if (basemap.layers[0].type == "googletrain"||basemap.layers[0].type == "googleimage") {
                  layer = new GoogleLayer();
                  layer.type = "googlepoi";
                  layer.label = basemap.title+"标注";
                  this.map.addLayer(layer, 1);
              }
          } else if (basemap.layers[0].type == "tianditumap" || basemap.layers[0].type == "tiandituimage" || basemap.layers[0].type == "tianditutrain") {
              layer = new TianDiTuLayer("w");
              layer.type = basemap.layers[0].type;
              layer.label = basemap.title;
              this.map.addLayer(layer, 0);
              if (basemap.layers[0].type == "tianditumap") {
                  layer = new TianDiTuLayer("w");
                  layer.type = "tianditumapi";
                  layer.label = basemap.title+"标注";
                  this.map.addLayer(layer, 1);
              } else if (basemap.layers[0].type == "tiandituimage") {
                  layer = new TianDiTuLayer("w");
                  layer.type = "tiandituimagei";
                  layer.label = basemap.title+"标注";
                  this.map.addLayer(layer, 1);
              } else if (basemap.layers[0].type == "tianditutrain") {
                  layer = new TianDiTuLayer("w");
                  layer.type = "tianditutraini";
                  layer.label = basemap.title+"标注";
                  this.map.addLayer(layer, 1);
              }
          } else if (basemap.layers[0].type == "a4map" || basemap.layers[0].type == "a4image" || basemap.layers[0].type == "a4train") {
              layer = new A4MapLayer();
              layer.type = basemap.layers[0].type;
              layer.label = basemap.title;
              this.map.addLayer(layer, 0);

              if (basemap.layers[0].type == "a4map") {
                  layer = new A4MapLayer();
                  layer.type = "a4mapi";
                  layer.label = basemap.title+"标注";
                  this.map.addLayer(layer, 1);
              } else if (basemap.layers[0].type == "a4image") {
                  layer = new A4MapLayer();
                  layer.type = "a4imagei";
                  layer.label = basemap.title+"标注";
                  this.map.addLayer(layer, 1);
              } else if (basemap.layers[0].type == "a4train") {
                  layer = new A4MapLayer();
                  layer.type = "a4traini";
                  layer.label = basemap.title+"标注";
                  this.map.addLayer(layer, 1);
