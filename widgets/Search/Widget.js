...
define([
  ...
  function(declare, lang, array, html, when, on, aspect, query, keys, Deferred, all,
    ...
    FeatureLayer, InfoTemplate, esriLang, utils,CinfoWindow,commonUtils,Graphic,
           ContentPane) {
    ...
    return declare([BaseWidget], {
      ...
      _onSelectResult: function(e) {
        ...
        this.publishData({
          ...
          //Portal修改
          var location,viewsPanel,graphicSymbol,graphic;
          if (result.feature.geometry.type === "point") {
              location = result.feature.geometry;
              viewsPanel="1,2,3";
              graphicSymbol=commonUtils.getDefaultSymbol('esriGeometryPoint');

              var alianValues= new Object();
              var layerFields=e.source.featureLayer.fields;
              for(var i=0;i<layerFields.length;i++){
                  alianValues[layerFields[i].name]=layerFields[i].alias;
              }
              var cinfoData={
                  dataValues:result.feature.attributes,
                  alianValues:alianValues,
                  viewsPanel:viewsPanel,
                  location:result.feature.geometry
              };
              this.map.infoWindow.setTitle(this.nls.alarm);
            ...
            /*this.map.infoWindow.setContent(cinfoWindow.domNode);
             this.map.infoWindow.show(location);*/
              ...
              var layerInfoTemplate =new InfoTemplate(this.nls.alarm,windowContent);//自定义地图info内容
              var infoGraphics=[new Graphic(location, null, null, layerInfoTemplate)];
              this.map.infoWindow.setFeatures(infoGraphics);
              this.map.infoWindow.show(location);

              graphic=new Graphic(result.feature.geometry,graphicSymbol);
              commonUtils.flashGraphic(graphic,this.map.graphics,true);
          }
      },
