...
define([
  ...
  ContentPane,
  CinfoWindow,
  InfoTemplate,SimpleMarkerSymbol,APDMOperation,commonUtils,MapPipelineStatus,CommonParams,PipelineObserver
) {
  return declare([_WidgetBase, Evented], {
    baseClass: 'jimu-widget-spacequerytable-feature-table',
    ...
    constructor: function(options) {
      ...
      this.set('geometryExtent', options.geometryExtent || null);
    ...
    startQuery: function(queryByStoreObjectIds) {
      if(this.layer)this.selectionManager.clearSelection(this.layer);
      this.cancelThread();
      ...
      this._currentDef.then(lang.hitch(this, function(layerObject) {
        ...
        var geoExtent = (!this.layerInfo.isTable && !this.matchingMap && this.geometryExtent) || null;//空间查询
        ...
        }else if (geoExtent && geoExtent.spatialReference && geoExtent.spatialReference.isWebMercator()) {
              var normalizeDef = this._currentDef = normalizeUtils.normalizeCentralMeridian(
                  [geoExtent], null, lang.hitch(this, function(normalizedGeo) {
                      return normalizedGeo[0];
                  }));
              normalizeDef.then(lang.hitch(this, function(_extent) {
                  this._doQuery(_extent, queryByStoreObjectIds);
    ...
    queryExecute: function(oFields, recordCounts, exceededLimit, results, nExtent) {
      ...
      columns = tableUtils.generateColumnsFromFields(
        ...
        this.layerInfoTemplateFields=[];
        this.layerInfoTemplateFields=results.fields;
    ...
    createTable: function(columns, store, recordCounts) {
      if (this.grid) {
        ...
        var demands = [OnDemandGrid, Selection, ColumnHider, ColumnResizer, ColumnReorder, Pagination, Selection];
        this.grid = new(declare(demands))(json, html.create("div"));
        html.place(this.grid.domNode, this.domNode);
        this.grid.startup();
        html.setStyle(this.grid.domNode,"height",  "calc(100% - 85px)");
      ...
      if (this.footer) {
        ...
        this.footer = html.create('div', {
          'class': 'jimu-widget-spacequerytable-feature-table-footer'
        }, this.domNode);
      }
    ...
    _showMapInfoWindowByFeatures: function(gExtent, result) {
      ...
      //var layerInfoTemplate = this.layerInfo.getInfoTemplate();
        ...
        var graphicAttributes=new Object();
        array.forEach(this.layerInfoTemplateFields, lang.hitch(this, function (field) {
            if(result[0].attributes[field['name']]){
                layerInfoTemplateStr+=("<b>"+field['alias']+"</b>"+":${"+field['name']+"}<br/>");
                graphicAttributes[field['alias']]=result[0].attributes[field['name']];
            }
        }));
        var layerInfoTemplate =new InfoTemplate(result[0].attributes.NAME||result[0].attributes.SITENAME||result[0].attributes.OBJECTID, layerInfoTemplateStr);//自定义地图info内容
      /*if (popup && popup.setFeatures && result.length === 1 && layerInfoTemplate) {
        ...
        if(this.layer.name=="天然气"||this.layer.name=="原油"||this.layer.name=="成品油"){
            var beginPoint=new Point(result[0].geometry.paths[0][0][0],result[0].geometry.paths[0][0][1],this.map.spatialReference);
            var pathsLength1=result[0].geometry.paths.length;
            var pathsLength2=result[0].geometry.paths[pathsLength1-1].length;
            var endPoint=new Point(result[0].geometry.paths[pathsLength1-1][pathsLength2-1][0],result[0].geometry.paths[pathsLength1-1][pathsLength2-1][1],this.map.spatialReference);
            var tolerance=commonUtils.pixelTpMapDistanceX(100,this.map);
            var apdmOperation=APDMOperation.getInstance({
                map:this.map
            });
            this.loading.show();
            apdmOperation.getPipePoint3byPoint2(beginPoint,endPoint,tolerance,CommonParams.getInstance().unitEventid,lang.hitch(this,this.linerouteExecuteComplete),lang.hitch(this,this.executeFault));
            ...
            var cinfoData={
                dataValues:graphicAttributes,
                viewsPanel:viewsPanel,
                location:result[0].geometry
            };
            this.map.infoWindow.setTitle(result[0].attributes.NAME||result[0].attributes.SITENAME||result[0].attributes.OBJECTID);
            ...
            var layerInfoTemplate =new InfoTemplate(result[0].attributes.NAME||result[0].attributes.SITENAME||result[0].attributes.OBJECTID,windowContent);//自定义地图info内容
            var infoGraphics=[new Graphic(location, null, null, layerInfoTemplate)];
            this.map.infoWindow.setFeatures(infoGraphics);
            this.map.infoWindow.show(location);
        }
    },
      linerouteExecuteComplete:function(event){
    ...
    selectFeatures: function(method, result) {
      if (result && result.length > 0) {
        if (method === "rowclick" || method === "selectall") {
          ...
          if(this.layer.name!="天然气"&&this.layer.name!="原油"&&this.layer.name!="成品油"){
              /*//自己添加Graphic
              var type = this.layer.geometryType;
              ...
              if (type === 'esriGeometryPoint') {
                  this.map.graphics.add(new Graphic(result[0].geometry,defaultPointSymbol));
              } else if (type === 'esriGeometryPolyline') {
                  this.map.graphics.add(new Graphic(result[0].geometry,defaultLineSymbol));
              } else if (type === 'esriGeometryPolygon') {
                  this.map.graphics.add(new Graphic(result[0].geometry,defaultFillSymbol));
              }*/
              this._setSelection(result);
          }
    ...
    _showColumnMenu: function(column, cell, target, coords) {
      ...
      var cm = new Menu({});
      html.addClass(cm.domNode, 'jimu-widget-spacequerytable-feature-menu');
      var that = this;
      if (info.sortable) {
        var labelArray = [
          this.nls.sortAsc,
