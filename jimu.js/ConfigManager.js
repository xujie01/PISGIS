      ...
      if(poolSectionConfig.groups){
        for(i = 0; i < poolSectionConfig.groups.length; i++){
          ...
          for(j = 0; j < poolSectionConfig.groups[i].widgets.length; j++){
            ...
            //popupWindow:设置弹出式窗口
            if(poolSectionConfig.groups[i].widgets[j].popupWindow=="true"){
                if(!poolSectionConfig.groups[i].panel){
                    poolSectionConfig.groups[i].panel = poolSectionConfig.panel;
                }

                if(poolSectionConfig.groups[i].panel && !poolSectionConfig.groups[i].panel.position){
                    poolSectionConfig.groups[i].panel.position = {};
                }

                if(!poolSectionConfig.groups[i].panel.position.relativeTo){
                    poolSectionConfig.groups[i].panel.position.relativeTo =
                        poolSectionConfig.groups[i].panel.positionRelativeTo?
                            poolSectionConfig.groups[i].panel.positionRelativeTo:'map';
                }
                ...
                poolSectionConfig.groups[i].widgets[j].panel = lang.clone(config.widgetOnScreen.panel);
                poolSectionConfig.groups[i].widgets[j].panel.position = poolSectionConfig.groups[i].widgets[j].position;
                poolSectionConfig.groups[i].widgets[j].panel.position.relativeTo =
                    poolSectionConfig.groups[i].widgets[j].position.relativeTo;
            }else{
                poolSectionConfig.groups[i].widgets[j].panel = poolSectionConfig.groups[i].panel;
            }
          }
      ...
      if(poolSectionConfig.widgets){
        for(i = 0; i < poolSectionConfig.widgets.length; i++){
          if(poolSectionConfig.widgets[i].inPanel === false){
            ...
            //popupWindow:设置弹出式窗口
            if(poolSectionConfig.widgets[i].popupWindow=="true"){
                if(!poolSectionConfig.widgets[i].position){
                    poolSectionConfig.widgets[i].position = {};
                }
                if(!poolSectionConfig.widgets[i].position.relativeTo){
                    poolSectionConfig.widgets[i].position.relativeTo =
                        poolSectionConfig.widgets[i] && poolSectionConfig.widgets[i].positionRelativeTo?
                            poolSectionConfig.widgets[i].positionRelativeTo: 'map';
                }
                if(poolSectionConfig.widgets[i].inPanel === true &&
                    !poolSectionConfig.widgets[i].panel){
                    poolSectionConfig.widgets[i].panel = lang.clone(config.widgetOnScreen.panel);
                    poolSectionConfig.widgets[i].panel.position = poolSectionConfig.widgets[i].position;
                    poolSectionConfig.widgets[i].panel.position.relativeTo =
                        poolSectionConfig.widgets[i].position.relativeTo;
                }
            }else{
                poolSectionConfig.widgets[i].panel = config.widgetPool.panel;
