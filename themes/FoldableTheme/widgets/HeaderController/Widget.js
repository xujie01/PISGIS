    ...
    var clazz = declare([BaseWidget, PoolControllerMixin], {
      ...
      startup: function() {
        ...
        // this.timeoutHandle = setTimeout(lang.hitch(this, this.resize), 100);
          ...
          //提前初始化，提前注册观察者类
          for(var i=0;i<this.appConfig.widgetOnScreen.widgets.length;i++){
              var widget=this.appConfig.widgetOnScreen.widgets[i];
              if(widget.id=="LocationQuery"){
          ...
          for(var i=0;i<this.appConfig.widgetPool.widgets.length;i++){
              ...
              if(widget.id=="widgets_Navigation_Widget"||widget.id=="widgets_FeatureQuery_Widget"||widget.id=="Monitor"){
                  WidgetManager.getInstance().loadWidget(widget);
              }
          }
          ...
          $("#OtherPanel").hover(lang.hitch(this,function(){
              this._onLogoMyClick();
          }),lang.hitch(this,function(){
              this._onLogoMyLeave();
          }));
        ...
        _onLogoMyLeave: function() {
            html.setStyle(this.OtherPanel, 'display', 'none');
        },
        _onthreeDType: function() {
            html.setStyle(this.OtherPanel, 'display', 'none');
        },
